...
public class ExpresionOralMam extends FlipperActivity implements OnInitializeComponent, View.OnClickListener{
    ...
    private ViewFlipper vfEvaExpresionOralMam;
    private float lastX;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.expresion_oral_mam);
        vfEvaExpresionOralMam = (ViewFlipper) findViewById(R.id.vfEvaExpresionOralMam);
        setOnInit(null);
        ...
        switch (touchevent.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (lastX < currentX) {
                    ...
                    if (vfEvaExpresionOralMam.getDisplayedChild() == 0)
                        break;

                    vfEvaExpresionOralMam.setInAnimation(this, R.anim.slide_in_from_left);

                    vfEvaExpresionOralMam.setOutAnimation(this, R.anim.slide_out_to_right);

                    vfEvaExpresionOralMam.showNext();
                }

                if (lastX > currentX) {

                    if (vfEvaExpresionOralMam.getDisplayedChild() == 1)
                        break;

                    vfEvaExpresionOralMam.setInAnimation(this, R.anim.slide_in_from_right);
                    vfEvaExpresionOralMam.setOutAnimation(this, R.anim.slide_out_to_left);

                    vfEvaExpresionOralMam.showPrevious();
                }
                break;
        }
        return false;
